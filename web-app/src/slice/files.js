...
const fileSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addMatcher(
                (action) =>
                    [saveUpdatingResourceMedia.fulfilled, saveUpdatingExplanationResourceContent.fulfilled].includes(action.type),
                (state, action) => {
                    state.loading = false;
            ...
            .addMatcher(
                (action) =>
                    [readExcelFile.pending, saveMultipleFiles.pending, saveUpdatingResourceMedia.pending, saveUpdatingExplanationResourceContent.pending].includes(action.type),
                (state) => {
                    state.loading = true;
                    state.error = null;
                }
